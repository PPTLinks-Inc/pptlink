...
export default function CourseStoreProvider({
  ...
  if (!storeRef.current) {
    storeRef.current = createStore<CourseStore>((set, get) => {
      const store: CourseStore = {
        ...
        togglePublish: async () => {
          ...
          if (err) {
            toast({
              title: "Error",
              description: !state.published ? "Failed to publish course, make sure all sections are complete" : "Failed to unpublish course",
              variant: "destructive"
            });
            return;
          }
      ...
      const isValid = (function validateInitialData() {
        const dateFrom = new Date(data.enrollmentDateFrom);
        dateFrom.setHours(0, 0, 0, 0);
        const dateTo = new Date(data.enrollmentDateTo);
        dateTo.setHours(0, 0, 0, 0);
        ...
        return !!(
          ...
          data.maxStudents &&
          data.thumbnail &&
          data.instructors?.every(
            (i) =>
              i.status === "APPROVED" &&
              i.instructor.role &&
              i.instructor.experience &&
              i.instructor.bio
          ) &&
          data.accountDetails &&
          data.CourseSection?.length > 0 &&
          data.CourseSection.every(
            (section) =>
              section.contents.length > 0 &&
              section.contents.every((content) => content.status === "done")
          )
