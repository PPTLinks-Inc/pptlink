...
export default function CourseCard({ img, type }) {
            ...
            >
                <span className="block !w-full h-fit mb-4 z-10 border-none rounded-t-[0.33rem] bg-[#FFFFF0]">
                    <span className={`w-[90%] mx-auto text-black font-bold py-1 flex justify-between items-center ${parseInt(type) % 2 === 0 && "justify-start"}`}>
                        <span>{parseInt(type) % 2 === 0 ? "Video" : "PPTFile"}</span>
                        <span className={`flex justify-between items-center gap-2 ${parseInt(type) % 2 === 0 && "hidden"}`}>
                            <span className={`block w-fit h-fit p-1 ${parseInt(type) !== 0 ? "bg-[red]" : "bg-gray-400"} border-none rounded-full`}></span>
                            <span>Live</span>
                        </span>
                    </span>
                </span>
                ...
                <div className="card_img rounded-lg border-[1px] border-solid border-slate-200 mx-4 maxScreenMobile:mx-2 relative">
                    <img
                        src={img}
                        alt={`${img} presentation thumbnail`}
                ...
                <div className={`card_body pb-5 maxScreenMobile:pb-2 text-white mx-4 maxScreenMobile:mx-2`}>
                ...
                <div className="w-[90%] maxScreenMobile:w-[95%] mb-2 mx-auto bg-[#FFFFF0] border-none rounded p-1 relative flex justify-start items-center">
                    ...
                    <span className="text-black !text-[0.7rem] font-bold my-[0.2px] z-10 absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center">16%</span>
                </div>
            </motion.div>
        </>
    );
